...
function Speedtest(workerFile = "speedtest_worker.js", type = "", debug = false) {
  ...
  // Construct the worker file URL with optional type and random query string
  let workerFileWithParams = this._workerFile;
  if (type !== "") {
    workerFileWithParams += `?r=${type}`;
  }
  if (!debug) {
    workerFileWithParams += (type !== "" ? "&" : "?") + `r=${Math.random()}`;
  }

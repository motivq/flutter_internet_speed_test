...
function Speedtest(workerFile = "speedtest_worker.js", type = "", debug = false) {
  this._serverList = []; //when using multiple points of test, this is a list of test points
  this._selectedServer = null; //when using multiple points of test, this is the selected server
  this._settings = {}; //settings for the speedtest worker
  this._state = 0; //0=adding settings, 1=adding servers, 2=server selection done, 3=test running, 4=done
  this._workerFile = workerFile; // Initialize _workerFile
  this._testType = type; // Initialize _testType
  this._workerFileWithParams = workerFile;


  if (type !== "") {
    this._workerFileWithParams += `?r=${type}`;
  }
  if (!debug) {
    this._workerFileWithParams += (type !== "" ? "&" : "?") + `r=${Math.random()}`;
  }

  this.worker = new Worker(this._workerFileWithParams);
}
...
const TestType = {
  ...
  IP: 'ip',
  PING: 'ping',
  CONTINUOUS_PING: 'continuousPing',
  ALL: 'all'
};
...
Speedtest.prototype = {
  ...
  setParameter: function(parameter, value) {
    ...
    // Allow setting the worker file location
    if (parameter === "workerFile") {
      this._workerFile = value;
      // Reinitialize the worker with the new file location
      let workerFileWithParams = this._workerFile || "speedtest_worker.js";
      if (this._settings.type !== "") {
        workerFileWithParams += `?r=${this._settings.type}`;
      }
      if (!this._settings.debug) {
        workerFileWithParams += (this._settings.type !== "" ? "&" : "?") + `r=${Math.random()}`;
      }
      //this.worker = new Worker(workerFileWithParams);
    }
  },
  /**
   * @returns {void}
  ...
  startDownloadTest: function() {
    this._testName = TestType.DOWNLOAD;
    this.startRequestedTest(TestType.DOWNLOAD);
  },
  startUploadTest: function() {
    this._testName = TestType.UPLOAD;
    this.startRequestedTest(TestType.UPLOAD);
  },
  startPingTest: function() {
    this._testName = TestType.PING;
    this.startRequestedTest(TestType.PING);
  },

  startContinuousPingTest: function() {
    this._testName = TestType.CONTINUOUS_PING;
    this.startRequestedTest(TestType.CONTINUOUS_PING, true);
  },
  ...
  startRequestedTest: function (testType) {
    ...
    this.worker.onmessage = function(e) {
      ...
      if (command === "finalPingTestResults") {
       // this._state = 4;
        //if (this.onend) this.onend(false, this._testName, null, null, null, null);
        console.log("finalPingTestResults command received");
      }
    ...
    switch (testType) {
      ...
      case TestType.PING:
        ...
        break;
      case TestType.CONTINUOUS_PING:
        this.worker.postMessage("startContinuousPing " + JSON.stringify(this._settings));
        break;
      case TestType.IP:
      ...
      switch (command) {
        ...
        case "ipTestResults":
          if (params[1] === this._prevData) return;
            ...
            //var data = JSON.parse(e.data.substring(command.length));
            //console.log(data);
            done(e.data.substring(command.length));
            break;
